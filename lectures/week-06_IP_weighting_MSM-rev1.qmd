...
\begin{tikzpicture}[node distance=2.5cm and 2.5cm, main/.style={draw, circle, minimum size=1.2cm}, text style/.style={text width=2.5cm, align=center}]
    ...
    \path[->, thick, bend left=25] (L0) edge (L1);
    \path[->, thick] (A0) edge (L1);
    \path[->, thick, bend left=25] (A0) edge (Y);
    \path[->, thick] (L1) edge (A1);
    \path[->, thick, bend right=35] (L1) edge (Y);
    \path[->, thick] (A1) edge (Y);
    
    % Annotation
    \node[text style, red] at (L1.north) [above=0.8cm] {\textbf{The Problem Variable}};
    \draw[->, red, thick] (L1.north) -- (L1.north |- L1.north) ++(0, 0.5);
    
    \node[text style, blue] at (2.5, -2.5) {Looking backward, $L_1$ is a \textbf{mediator} of $A_0$'s effect. Adjusting for it blocks this causal path.};
    \draw[->, blue, thick, dashed] (A0.south) to[bend right=20] (L1.south);

    \node[text style, red] at (7.5, -3) {Looking forward, $L_1$ is a \textbf{confounder} for $A_1$'s effect. We must adjust for it to block the backdoor path.};
    \draw[->, red, thick] (L1.south) to (A1.south);
    \draw[->, red, thick] (L1.south) to[bend right=25] (Y.south);
\end{tikzpicture}
```
